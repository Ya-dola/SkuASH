...
public class AutoSave : Editor
{
    ...
    private static async Task SaveInterval(CancellationToken token)
    {
        ...
        {
            await Task.Delay(_config.frequencyMins * 1000 * 60, token);
            ...
            if (_config.logSave) Debug.Log($"Auto-saved at {DateTime.Now:h:mm:ss tt}");
        }
    }
    ...
    public override void OnInspectorGUI()
    {
        ...
        //     MessageType.Info);
        EditorGUILayout.HelpBox("(✿◡‿◡) - Yadola",
            MessageType.Info);
    }
}